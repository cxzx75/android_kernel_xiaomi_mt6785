...
static enum power_supply_property battery_props[] = {
	...
	POWER_SUPPLY_PROP_MAIN_CAM_TEMP,
	POWER_SUPPLY_PROP_PD_CHARGER_TEMP,
...
/* 2021.02.04 longcheer jiangshitian change for pd-chg and main-cam thermal begin */
#if defined(CONFIG_TARGET_PROJECT_K7B)
extern int get_hw_main_cam_temp(void);
extern int get_hw_pd_charger_temp(void);
#endif
/* 2021.02.04 longcheer jiangshitian change for pd-chg and main-cam thermal end */
		...
		enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_RESISTANCE_ID:
		...
		case 3:
			val->intval = 1800000;
			break;
		/*2020.12.29 longcheer jiangshitian edit for P1 bat id/type check end*/
		default:
			break;
		}
		break;
		...
		enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		...
		break;
	case POWER_SUPPLY_PROP_RESISTANCE_ID:
		pr_err("%s : %d : POWER_SUPPLY_PROP_RESISTANCE_ID  gm.battery_id :%d.\n", __func__, __LINE__, gm.battery_id);
		switch (gm.battery_id) {
		case 0:
			val->intval = 68000;
			break;
	...
	case POWER_SUPPLY_PROP_RESISTANCE:
		...
		break;
	case POWER_SUPPLY_PROP_BATTERY_TYPE:
		pr_err("%s : %d : POWER_SUPPLY_PROP_BATTERY_TYPE  gm.battery_id :%d.\n", __func__, __LINE__, gm.battery_id);
		switch (gm.battery_id) {
		/*2020.12.29 longcheer jiangshitian edit for P1 bat id/type check start*/
		case 0:
			val->strval = "k7b_nvt";
	...
	union power_supply_propval *val)
{
	...
	/* 2021.02.04 longcheer jiangshitian change for pd-chg and main-cam thermal begin */
	#if defined(CONFIG_TARGET_PROJECT_K7B)
	case POWER_SUPPLY_PROP_MAIN_CAM_TEMP:
		//pr_err("%s  %d POWER_SUPPLY_PROP_MAIN_CAM_TEMP ...... \n", __func__, __LINE__);
		val->intval = get_hw_main_cam_temp();
		break;
	case POWER_SUPPLY_PROP_PD_CHARGER_TEMP:
		//pr_err("%s  %d POWER_SUPPLY_PROP_PD_CHARGER_TEMP ...... \n", __func__, __LINE__);
		val->intval = get_hw_pd_charger_temp();
		break;
	case POWER_SUPPLY_PROP_SET_TEMP_ENABLE:
		//pr_err("%s  %d POWER_SUPPLY_PROP_MAIN_CAM_TEMP ...... \n", __func__, __LINE__);
		val->intval = charger_manager_get_prop_set_temp_enable();
		break;
	case POWER_SUPPLY_PROP_SET_TEMP_NUM:
		//pr_err("%s  %d POWER_SUPPLY_PROP_MAIN_CAM_TEMP ...... \n", __func__, __LINE__);
		val->intval = charger_manager_get_prop_set_temp_num();
		break;
	#endif
	/* 2021.02.04 longcheer jiangshitian change for pd-chg and main-cam thermal end */
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		...
		#else
		ret = power_supply_get_property(psy,POWER_SUPPLY_PROP_CHARGING_ENABLED, &chg_flag_unin);
		if(chg_flag_unin.intval)
		{
			val->intval = 0;
		}
		else
	...
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		val->intval = 5000000;      //androidR update mah to uah
			...
			const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PD_CHARGER_TEMP:
		...
		break;
	case POWER_SUPPLY_PROP_SET_TEMP_ENABLE:
		charger_manager_set_prop_set_temp_enable(val->intval);
		break;
	case POWER_SUPPLY_PROP_SET_TEMP_NUM:
		charger_manager_set_prop_set_temp_num(val->intval);
		break;
	#endif
	/* 2021.02.04 longcheer jiangshitian change for pd-chg and main-cam thermal end */
	default:
...
int force_get_tbat_internal(bool update)
{
	...
	set_temp_enabled = charger_manager_get_prop_set_temp_enable();
	if(set_temp_enabled == 1)
		bat_temperature_val = charger_manager_get_prop_set_temp_num();

	bm_err("[force_get_tbat][err] bat_temperature_val:%d , set_temp_enabled :%d\n",bat_temperature_val,set_temp_enabled);
