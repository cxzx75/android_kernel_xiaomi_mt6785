...
// xiaomi Add
int adapter_dev_set_cap_bq(struct adapter_device *adapter_dev,
	enum adapter_cap_type type,
	int mV, int mA)
{
	if (adapter_dev != NULL && adapter_dev->ops != NULL &&
	    adapter_dev->ops->set_cap_bq)
		return adapter_dev->ops->set_cap_bq(adapter_dev, type, mV, mA);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(adapter_dev_set_cap_bq);
// xiaomi Add
...
static struct attribute *adapter_class_attrs[] = {
	&dev_attr_name.attr,
	&dev_attr_request_vdm_cmd.attr,// xiaomi Add
	&dev_attr_current_state.attr,// xiaomi Add
	&dev_attr_adapter_id.attr,// xiaomi Add
	&dev_attr_adapter_svid.attr,// xiaomi Add
	//&dev_attr_adapter_version.attr,// xiaomi Add
	&dev_attr_verify_process.attr,// xiaomi Add
	&dev_attr_usbpd_verifed.attr,// xiaomi Add
	&dev_attr_current_pr.attr,// xiaomi Add
	&dev_attr_pdos[0].attr,// xiaomi Add
	&dev_attr_pdos[1].attr,// xiaomi Add
	&dev_attr_pdos[2].attr,// xiaomi Add
	&dev_attr_pdos[3].attr,// xiaomi Add
	&dev_attr_pdos[4].attr,// xiaomi Add
	&dev_attr_pdos[5].attr,// xiaomi Add
	&dev_attr_pdos[6].attr,// xiaomi Add
	NULL,
};
