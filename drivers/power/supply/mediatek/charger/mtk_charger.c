...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	if (info && info->usb_psy && info->bms_psy && info->data.enable_ffc) {
		...
		if (pd_authentication || charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3P5) {
			...
			} else if ((info->battery_temp <= info->data.temp_t3_thres_minus_x_degree) &&
			...
			if (info->data.ffc_ieoc_warm != info->data.ffc_ieoc && fastcharge_mode) {
	...
	chr_err("[SW_JEITA]preState:%d newState:%d tmp:%d cv:%d,%d jeita_current_limit:%d\n",
...
{
	union power_supply_propval val;
	int type_mode = 0, real_type = 0 , battery_status = 0;
	...
	power_supply_get_property(pinfo->battery_psy, POWER_SUPPLY_PROP_STATUS, &val);
	...
	if (real_type == POWER_SUPPLY_TYPE_UNKNOWN && (type_mode == POWER_SUPPLY_TYPEC_NONE || type_mode == POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER)) {
	...
	}else if (real_type == POWER_SUPPLY_TYPE_USB_PD){
		if (battery_status == POWER_SUPPLY_STATUS_FULL)
			charger_manager_notifier(pinfo, CHARGER_NOTIFY_EOC);
		...
		else if(battery_status == POWER_SUPPLY_STATUS_DISCHARGING)
			charger_manager_notifier(pinfo, CHARGER_NOTIFY_STOP_CHARGING);
		else
			charger_manager_notifier(pinfo, CHARGER_NOTIFY_START_CHARGING);
	} else
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "check_hv_current", &val) >= 0)
		info->data.check_hv_current = val;
	else {
		chr_err("use default CHECK_HV_CURRENT:%d\n",
			CHECK_HV_CURRENT);
		info->data.check_hv_current = CHECK_HV_CURRENT;
	}
