...
{
	*state = 1;
	mtk_cooler_3Gmutt_dprintk("%s() %s %lu\n", __func__,
							cdev->type, *state);
	return 0;
}
...
struct thermal_cooling_device *cdev, unsigned long state)
{
	mtk_cooler_3Gmutt_dprintk("%s() %s %lu pid=%d\n", __func__,
...
static int _mtk_cl_mutt_proc_read(struct seq_file *m, void *v)
{
	 ...
	 */
	{
		...
		for (; i < MAX_NUM_INSTANCE_MTK_COOLER_3GMUTT; i++) {
			...
			MTK_CL_3GMUTT_GET_CURR_STATE(curr_state,
							cl_mutt_state[i]);
...
struct file *filp, const char __user *buffer, size_t count, loff_t *data)
{
	int len = 0;
	char desc[128];
	int klog_on, mutt0_a, mutt0_s, mutt1_a, mutt1_s,
			mutt2_a, mutt2_s, mutt3_a, mutt3_s;
	...
	/**
	 * sscanf format
	 * <klog_on> <mtk-cl-mutt00 active (ms)> <mtk-cl-mutt00 suspended (ms)>
	 ...
	 * <mtk-cl-mutt* active/suspended (ms) > can only be positive integer
	 *						or 0 to denote no limit
	 */
	...
	if (sscanf(desc, "%d %d %d %d %d %d %d %d %d", &klog_on, &mutt0_a,
				&mutt0_s, &mutt1_a, &mutt1_s,
		...
		&& mutt0_s >= 100 && mutt0_s <= 25500)
			cl_mutt_param[0] = ((mutt0_s / 100) << 16)
						| ((mutt0_a / 100) << 8) | 1;
		...
		&& mutt1_s >= 100 && mutt1_s <= 25500)
			cl_mutt_param[1] = ((mutt1_s / 100) << 16)
						| ((mutt1_a / 100) << 8) | 1;
		...
		&& mutt2_s >= 100 && mutt2_s <= 25500)
			cl_mutt_param[2] = ((mutt2_s / 100) << 16)
						| ((mutt2_a / 100) << 8) | 1;
		...
		&& mutt3_s >= 100 && mutt3_s <= 25500)
			cl_mutt_param[3] = ((mutt3_s / 100) << 16)
						| ((mutt3_a / 100) << 8) | 1;
...
static int cl_debug_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "[ %s] cl_debug_flag = %d\n", __func__, cl_debug_flag);


	return 0;
}
static ssize_t cl_debug_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d %d", &cl_debug_flag_switch,
	&cl_debug_flag_switch1, &cl_debug_flag_switch2) == 3) {
		cl_debug_flag = cl_debug_flag_switch;
		mtk_cooler_3Gmutt_dprintk("%s cl_debug_flag=%d\n", __func__,
								cl_debug_flag);

		/*return count;*/
	} else {
		mtk_cooler_3Gmutt_dprintk("%s bad argument\n", __func__);
	}
	return -EINVAL;
