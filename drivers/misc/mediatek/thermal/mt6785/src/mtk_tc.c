...
static void thermal_interrupt_handler(unsigned int tc_num)
{
	...
	tscpu_printk("[Thermal IRQ] Auxadc thermal controller %d, TEMPMONINTSTS=0x%08x\n",
		tc_num, ret);

	if (ret & THERMAL_COLD_INTERRUPT_0)
		tscpu_dprintk("[Thermal IRQ]: Cold interrupt triggered, sensor point 0\n");

	if (ret & THERMAL_HOT_INTERRUPT_0)
		tscpu_dprintk("[Thermal IRQ]: Hot interrupt triggered, sensor point 0\n");

	if (ret & THERMAL_LOW_OFFSET_INTERRUPT_0)
		tscpu_dprintk("[Thermal IRQ]: Low offset interrupt triggered, sensor point 0\n");

	if (ret & THERMAL_HIGH_OFFSET_INTERRUPT_0)
		tscpu_dprintk("[Thermal IRQ]: High offset interrupt triggered, sensor point 0\n");

	if (ret & THERMAL_HOT2NORMAL_INTERRUPT_0)
		tscpu_dprintk("[Thermal IRQ]: Hot to normal interrupt triggered, sensor point 0\n");

	if (ret & THERMAL_COLD_INTERRUPT_1)
		tscpu_dprintk("[Thermal IRQ]: Cold interrupt triggered, sensor point 1\n");

	if (ret & THERMAL_HOT_INTERRUPT_1)
		tscpu_dprintk("[Thermal IRQ]: Hot interrupt triggered, sensor point 1\n");

	if (ret & THERMAL_LOW_OFFSET_INTERRUPT_1)
		tscpu_dprintk("[Thermal IRQ]: Low offset interrupt triggered, sensor point 1\n");

	if (ret & THERMAL_HIGH_OFFSET_INTERRUPT_1)
		tscpu_dprintk("[Thermal IRQ]: High offset interrupt triggered, sensor point 1\n");

	if (ret & THERMAL_HOT2NORMAL_INTERRUPT_1)
		tscpu_dprintk("[Thermal IRQ]: Hot to normal interrupt triggered, sensor point 1\n");

	if (ret & THERMAL_COLD_INTERRUPT_2)
		tscpu_dprintk("[Thermal IRQ]: Cold interrupt triggered, sensor point 2\n");

	if (ret & THERMAL_HOT_INTERRUPT_2)
		tscpu_dprintk("[Thermal IRQ]: Hot interrupt triggered, sensor point 2\n");

	if (ret & THERMAL_LOW_OFFSET_INTERRUPT_2)
		tscpu_dprintk("[Thermal IRQ]: Low offset interrupt triggered, sensor point 2\n");

	if (ret & THERMAL_HIGH_OFFSET_INTERRUPT_2)
		tscpu_dprintk("[Thermal IRQ]: High offset interrupt triggered, sensor point 2\n");

	if (ret & THERMAL_HOT2NORMAL_INTERRUPT_2)
		tscpu_dprintk("[Thermal IRQ]: Hot to normal interrupt triggered, sensor point 2\n");

	if (ret & THERMAL_AHB_TIMEOUT_INTERRUPT)
		tscpu_dprintk("[Thermal IRQ]: AHB bus timeout triggered\n");

	if (ret & THERMAL_IMMEDIATE_INTERRUPT_0)
		tscpu_dprintk("[Thermal IRQ]: Immediate sense interrupt triggered, sensor point 0\n");

	if (ret & THERMAL_IMMEDIATE_INTERRUPT_1)
		tscpu_dprintk("[Thermal IRQ]: Immediate sense interrupt triggered, sensor point 1\n");

	if (ret & THERMAL_IMMEDIATE_INTERRUPT_2)
		tscpu_dprintk("[Thermal IRQ]: Immediate sense interrupt triggered, sensor point 2\n");

	if (ret & THERMAL_FILTER_INTERRUPT_0)
		tscpu_dprintk("[Thermal IRQ]: Filter sense interrupt triggered, sensor point 0\n");

	if (ret & THERMAL_FILTER_INTERRUPT_1)
		tscpu_dprintk("[Thermal IRQ]: Filter sense interrupt triggered, sensor point 1\n");

	if (ret & THERMAL_FILTER_INTERRUPT_2)
		tscpu_dprintk("[Thermal IRQ]: Filter sense interrupt triggered, sensor point 2\n");

	if (ret & THERMAL_COLD_INTERRUPT_3)
		tscpu_dprintk("[Thermal IRQ]: Cold interrupt triggered, sensor point 3\n");

	if (ret & THERMAL_HOT_INTERRUPT_3)
		tscpu_dprintk("[Thermal IRQ]: Hot interrupt triggered, sensor point 3\n");

	if (ret & THERMAL_LOW_OFFSET_INTERRUPT_3)
		tscpu_dprintk("[Thermal IRQ]: Low offset interrupt triggered, sensor point 3\n");

	if (ret & THERMAL_HIGH_OFFSET_INTERRUPT_3)
		tscpu_dprintk("[Thermal IRQ]: High offset triggered, sensor point 3\n");

	if (ret & THERMAL_HOT2NORMAL_INTERRUPT_3)
		tscpu_dprintk("[Thermal IRQ]: Hot to normal interrupt triggered, sensor point 3\n");

	if (ret & THERMAL_IMMEDIATE_INTERRUPT_3)
		tscpu_dprintk("[Thermal IRQ]: Immediate sense interrupt triggered, sensor point 3\n");

	if (ret & THERMAL_FILTER_INTERRUPT_3)
		tscpu_dprintk("[Thermal IRQ]: Filter sense interrupt triggered, sensor point 3\n");

	if (ret & THERMAL_PROTECTION_STAGE_1)
		tscpu_dprintk("[Thermal IRQ]: Thermal protection stage 1 interrupt triggered\n");

	if (ret & THERMAL_PROTECTION_STAGE_2) {
		tscpu_dprintk("[Thermal IRQ]: Thermal protection stage 2 interrupt triggered\n");
#if MTK_TS_CPU_RT
	...
	if (ret & THERMAL_PROTECTION_STAGE_3) {
		tscpu_printk("[Thermal IRQ]: Thermal protection stage 3 interrupt triggered, Thermal HW reboot\n");
