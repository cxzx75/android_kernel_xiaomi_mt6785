...
static void lcm_get_params(struct LCM_PARAMS *params)
{
	...
	params->dsi.horizontal_backporch = 50;//40
...
/*
struct LCM_setting_table switch_table_page_2[] = {
	{ 0xF0, 2, {0x5A,0x59} }
};
*/
...
/*	unsigned int id = 0;
...
#if defined(BUILD_LK)
	printf("%s, kxx add for vtr gpio68= %d \n", __func__, ret);
#endif
	if(ret == 1)
	{
	array[0] = 0x00023700;
	...
	pr_debug("%s,kxx add for vtdr_id=0x%x,version_id=0x%x\n", __func__, id, version_id);

	if (id == 0x06 && version_id ==0x11)
		return 1;
	else
		return 0;
	}
...
struct LCM_DRIVER dsi_panel_k7_44_0e_0b_fhdp_video = {
	.name = "dsi_panel_k7_44_0e_0b_fhdp_video",
