...
int elliptic_open_port(int portid)
{
    //return elliptic_io_open_port(portid);
       int ret = 0;
       //ret |= elliptic_io_open_port(portid);
       pr_debug("ELUS sensor_enable_to_hub (1)");
       ret = sensor_enable_to_hub(ID_ELLIPTIC_FUSION, 1);
...
int elliptic_close_port(int portid)
{
    //return elliptic_io_close_port(portid);
       int ret = 0;
       pr_debug("ELUS sensor_enable_to_hub (1)");
       ret = sensor_enable_to_hub(ID_ELLIPTIC_FUSION, 0);
       //ret |= elliptic_io_close_port(portid);
