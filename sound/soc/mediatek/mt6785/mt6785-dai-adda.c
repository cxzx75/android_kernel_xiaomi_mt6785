...
static int mtk_adda_ul_src_dmic(struct mtk_base_afe *afe, int id)
{
	...
	case MT6785_DAI_AP_DMIC:
		reg = AFE_ADDA_UL_SRC_CON0;
		break;
	case MT6785_DAI_ADDA_CH34:
	case MT6785_DAI_AP_DMIC_CH34:
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap, AFE_ADDA_MTKAIF_RX_CFG0,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6785_DAI_ADDA);
		}
		break;
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap,
					   ...
					   MTKAIF_RXIF_VOICE_MODE_MASK_SFT,
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6785_DAI_ADDA_CH34);
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
...
/* mtkaif dmic */
static const char * const mt6785_adda_off_on_str[] = {
	"Off", "On"
};

static const struct soc_enum mt6785_adda_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6785_adda_off_on_str),
			    mt6785_adda_off_on_str),
};

static int mt6785_adda_dmic_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6785_adda_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_adda_controls[] = {
	...
	SOC_ENUM_EXT("MTKAIF_DMIC", mt6785_adda_enum[0],
		     mt6785_adda_dmic_get, mt6785_adda_dmic_set),
};
...
int mt6785_dai_adda_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6785_dai_set_priv(afe, MT6785_DAI_ADDA,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	ret = mt6785_dai_set_priv(afe, MT6785_DAI_ADDA_CH34,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	/* ap dmic priv share with adda */
	afe_priv->dai_priv[MT6785_DAI_AP_DMIC] =
		afe_priv->dai_priv[MT6785_DAI_ADDA];
	afe_priv->dai_priv[MT6785_DAI_AP_DMIC_CH34] =
		afe_priv->dai_priv[MT6785_DAI_ADDA_CH34];
